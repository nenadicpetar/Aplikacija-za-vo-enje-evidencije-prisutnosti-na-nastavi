    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btLogin.setOnClickListener {
            ...
            if (edtMkpt.text.toString().equals("") || edtPw.text.toString().equals("")) {
                Toast.makeText(applicationContext, "Unesite mkpt i lozinku", Toast.LENGTH_LONG).show()
            } else if (edtMkpt.text.length < 5 || !edtMkpt.text.contains("mkpt")) {
                Toast.makeText(applicationContext, "Unesite ispravan mkpt-0000", Toast.LENGTH_LONG).show()
            } else {

                var mkptRaw = edtMkpt.text.toString()
                var mkpt = mkptRaw.substring(5)

                Log.d("Petar", "dijete.........${mkpt}")
                ...
                studenttable.child("ams").child("student").child(mkpt).addListenerForSingleValueEvent(object : ValueEventListener{
                    ...
                    override fun onDataChange(dataSnapshot: DataSnapshot) {
                        ...
                        if (dataSnapshot.exists()) {
                            ...
                            Log.d("Petar", "ime: ${student!!.name} i lozinka: ${student!!.password}")
                            if (edtPw.text.toString().equals(decryptPassword)) {

                                //Log.d("Petar", "${student.name}, ${student.mkpt},${student.subject} ,${student.phone}, ${student.email}, ${student.address}, ${student.profileurl}")

                                //saving current user account for other activity

                                // var prefEditor = sharedPreferences.edit()
                                //prefEditor.clear()
                                prefEditor.putString("ime", student?.name)
                                prefEditor.putString("mkpt", student?.mkpt)
                                prefEditor.putString("izborni blok", student?.major)
                                prefEditor.putString("kolegij", student?.subject)
                                prefEditor.putString("telefon", student?.phone)
                                prefEditor.putString("e-mail", student?.email)
                                prefEditor.putString("adresa", student?.address)
                                prefEditor.putString("profileUrl", student?.profileurl)
                                prefEditor.putString("lozinka", student?.password)
                                // prefEditor.clear()
                                // prefEditor.commit()
                                // prefEditor.apply()
