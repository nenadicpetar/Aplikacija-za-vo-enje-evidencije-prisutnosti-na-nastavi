        ...
        btCreate.setOnClickListener {
            ...
            if(password.equals(confirmPassword)) {
                ...
                AlertDialog.Builder(this)
                    .setTitle("Potvrda")
                    .setMessage("Jeste li sigurni da želite kreirati račun? ")
                    .setPositiveButton("Prijavite se", DialogInterface.OnClickListener { dialogInterface, i ->
                        dialogInterface.dismiss()
                        //new dialog

                        var studentTable = FirebaseDatabase.getInstance().reference.child("ams").child("student")
                        studentTable.child(sharedPreferences!!.getString(getString(R.string.prefMkpt), null)!!).addListenerForSingleValueEvent(object :
                            ValueEventListener {
                            ...
                            override fun onDataChange(dataSnapshot: DataSnapshot) {
                                if (dataSnapshot.exists()){
                                    AlertDialog.Builder(this@CreatePasswordActivity)
                                        .setTitle("Greška")
                                        .setMessage("Korisnik već postoji. Kontaktirajte administratora ili provjerite ID broj i pokušajte ponovno!")
                                        .setPositiveButton("U redu", DialogInterface.OnClickListener { dialogInterface, i ->dialogInterface.dismiss()
                                        })
                                        .show()
                                    //Toast.makeText(applicationContext, "Korisnik s istim ID-em već postoji. Kontaktirajte administratora ili provjerite ID broj i pokušajte ponovno!",Toast.LENGTH_LONG).show()
                                }else{
                                    var encryptPassword = confirmPassword.encrypt("amsucsmapp123456")
                                    Log.d("Petar","Enkriptirana lozinka je $encryptPassword i dekriptirana je ${encryptPassword.decrypt("amsucsmapp123456")}")
                                    ...
                                    Toast.makeText(applicationContext, "Račun kreiran!",Toast.LENGTH_LONG).show()

                                    AlertDialog.Builder(this@CreatePasswordActivity)
                                        .setTitle("Uspjeh")
                                        .setMessage("Čestitamo! Vaš račun je kreiran.")
                                        .setPositiveButton("U redu", DialogInterface.OnClickListener { dialogInterface, i ->
                                            dialogInterface.dismiss()
                    ...
                    .setNegativeButton("Prekini", DialogInterface.OnClickListener { dialogInterface, i -> dialogInterface.dismiss() })
                    .show()
            }else{
                AlertDialog.Builder(this)
                    .setTitle("Greška")
